    ...
    return LaunchDescription(
        [
            Node(
                ...
                parameters=[{"timer_period_sec": 0.01}],
...
class TalkerTest(unittest.TestCase):
    ...
    def test_talker_publish_frequency(self, proc_output):
        ...
        subscriber = self.node.create_subscription(
            std_msgs.msg.String,
            "chatter",
            lambda message: messages_rx.append(message),
            10,
        )
    ...
    def test_talker_message_increment(self, proc_output):
        ...
        subscriber = self.node.create_subscription(
            ...
            "chatter",
            lambda message: (
                values_rx.append(int(message.data.split(": ")[1]))
                if message.data.startswith("Hello, world: ")
                and message.data.split(": ")[1].isdigit()
                else None
            ),
            10,
        )
        ...
        try:
            ...
            self.assertTrue(
                all(
                    values_rx[i] == values_rx[i - 1] + 1
                    for i in range(1, len(values_rx))
                )
            )
        finally:
            self.node.destroy_subscription(subscriber)
